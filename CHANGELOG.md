...
- `1.1.0` is the new alignment point for the private Dev library and the public Patina repository.
- Minor version bump is intentional because the library surface now includes new filter circuit APIs and broader filter-engine coverage.

### Public API Aggregation

- Updated `include/patina.h` so the aggregate header exports the new filter circuits:
  - `dsp/circuits/filters/OtaLadderFilter.h`
  - `dsp/circuits/filters/AcidLadderFilter.h`
  - `dsp/circuits/filters/AllPassFilter.h`
- This fixes the mismatch where the new headers existed in Dev but were not available through the single public include.

### New / Newly Inventoried Filter Work

- Added `OtaLadderFilter`: 4-pole OTA ladder filter with ZDF/TPT integration, per-stage mismatch, temperature drift, and OTA saturation behavior.
- Added `AcidLadderFilter`: 3-pole transistor ladder variant with input HPF, resonance-dependent BP blend, drive behavior, and ZDF/TPT integration.
- Added `AllPassFilter`: 1st-order TPT all-pass section cascade, up to four stages, for phase-shift and phaser-style building blocks.
- Added tests for `AllPassFilter` and `OtaLadderFilter`, and registered them in `tests/CMakeLists.txt`.
...
- Optimized analog noise and drift processing by replacing per-sample `std::normal_distribution` calls with lower-rate Gaussian updates where the modulation source does not need per-sample random distribution construction.
- The change preserves the intended analog drift/noise character while reducing audio-thread RNG overhead.

### Experiment

- `VocoderBand` is explicitly classified as **Experiment**.
- It remains available for evaluation, tests, and exploratory integration, but release communication should not present it as a stable production vocoder engine yet.

### Repository Hygiene

- Public tree remains free of local macOS `.DS_Store` metadata.
- Added `docs/RELEASE_INVENTORY_2026-07-08.md` to document Dev/Public roles, version alignment, newly inventoried work, and the vocoder experiment status.

---

## 2026-04-15 — Vocoder Module, Temperature Modeling & Filter Improvements

### New: VocoderBand (L3 Vocoder Circuit) (`new module`)

Complete analog vocoder band circuit implementation from L2 primitives:
- **Signal path:** Modulator (voice) → OTA SVF BPF → Half-wave rectifier → RC envelope detector → VCA (carrier modulation)
- **Components:** OTA_Primitive ×2, DiodePrimitive, RC_Element ×2, VcaPrimitive
- **Temperature-dependent:** OTA transconductance and VCA gain scale with chassis temperature
- **Attack/Release:** Configurable asymmetric RC charging (fast attack) and discharging (slow release)
- **Files added:** `dsp/circuits/vocoder/VocoderBand.h`, `tests/test_vocoder_band.cpp`
- **Include updated:** `include/patina.h` now exports `VocoderBand`
- **Documented:** New section 62 in API_REFERENCE.md

### Improvement: TubePreamp Temperature Modeling

**Enhanced `TubePreamp` with operating temperature (°C) parameter:**

```cpp
float temperature = 25.0f;  // Default 25°C (room temperature, no change vs existing)
```

**Temperature effects:**
- **Transconductance (gm):** Nonlinear curve peaking at ~55°C (+4%), dropping at cold temps (−3.5% at 15°C)
- **Microphonics:** Scale factor up to 2× at 100°C (thermal expansion of tube elements)
- **Grid conduction:** Sensitivity increases ~0.5%/°C above 25°C baseline
- **Grid switching:** New `setTube()` method allows switching tube type (12AX7 ↔ 12AT7) at runtime
- **Params struct:** Added `tubeAge` (0–1) and `supplyRipple` (0–1) parameters

**Files changed:** `dsp/circuits/saturation/TubePreamp.h`

### Improvement: PassiveLCFilter — TPT/ZDF SVF Upgrade

**Upgraded from simple SVF to unconditionally stable TPT (Thierry Parasol Topology) state-variable filter:**

**Why:** 
- Simple SVF diverges when Q is high or center frequency approaches Nyquist (fc > 20 kHz)
- TPT form (Zavalishin 2012) eliminates delay-free loops; guaranteed stable for all Q and fc

**Behavior change:**
- Phase response flattened near Nyquist (previously warped at extreme frequencies)
- Resonant peak sharpening at high Q more accurate
- Improved stability in near-Nyquist sweepsFilter outputs (`LPF`, `HPF`, `BPF`, `Notch`) nomenclature preserved; backward-compatible API

**Files changed:** `dsp/circuits/filters/PassiveLCFilter.h`

### Docs: API_REFERENCE Vocoder & Section Renumbering

- Added new section 62: `VocoderBand` with full API, usage examples, and vocoder topology diagram
- Renumbered all sections from Mixer onwards (62→63, 63→64, ... , 81→82, 82→83, etc.)
- Updated TOC and internal anchor links for sections 63–84 (Mixer through Rust Crate)
- Updated TubePreamp documentation with new temperature parameter effects table
...
| `DiodeLadderFilter` | Diode ladder filter (3-pole, resonant character) |
